...
\begin{table}[t]
	...
	\begin{tabular}{l p{2.5cm}}
		...
		\ac{RL} Game & -0.282\textsuperscript{***} \\
		Rules Only & -0.305\textsuperscript{***} \\
		Moderation Guid. & -0.107\textsuperscript{*} \\
		Facilitation Guid. & -0.007\textsuperscript{} \\
		time & -0.012\textsuperscript{**} \\
		No Instructions$\times$time & 0.003 \\
		\ac{RL} Game$\times$time & 0.003 \\
		Rules Only$\times$time & -0.002 \\
		Moderation Guid.$\times$time & -0.011\textsuperscript{*} \\
		Facilitation Guid.$\times$time & -0.024\textsuperscript{***} \\
		\bottomrule
	\end{tabular}
	\small
	\asterisknote
	\normalsize
	\caption{\ac{OLS} regression coefficients for Arg.Q. (Adj.R2=0.016). \textit{“Time”} denotes dialogue turn, reference factor is \textit{“No facilitator”}.}
	\label{tab:argq}
\end{table}
...
\begin{figure}[t]
	\centering
	\includegraphics[width=0.8\linewidth]{toxicity_aq.png}
